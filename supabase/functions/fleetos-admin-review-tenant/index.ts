/**
 * FleetOS P1.1 — admin approve / reject company application.
...
} from '../_shared/fleetos-admin-review.ts';
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const parsed = parseReviewTenantBody(body);
  if (!parsed.ok) {
  ...
  const result = await reviewTenantApplication(admin, {
    ...parsed.input,
    reviewerSource: auth.reviewerSource,
  ...
  return json(200, buildReviewHttpBody(result.data), cors);
});
